...
<head>
    ...
    <div class="list">
        <div class="input-search">
            <input type="text" class="txtb" id="search" placeholder="搜索文章名：">
        ...
        <div class="notcomp">
            ...
            <c:forEach items="${requestScope.pageArticle.pageList}" var="article">
                <div class="task" id="div${article.articleId}">
                    ...
                    <span class="time">发布时间: <fmt:formatDate value="${article.articleDate}" pattern="yyyy-MM-dd HH:mm:ss"></fmt:formatDate> </span>
                    ...
                    <c:if test="${sessionScope.USER.userId == article.authorId || sessionScope.USER.userStatus == 2 }">
                        <i class="iconfont icon-shanchu1 deleteIcon"  name="${article.articleId}" onclick="deleteIconOnClick(this)"></i>
                    </c:if>
                </div>
            </c:forEach>
        </div>
        <c:if test="${requestScope.pageArticle.totalPage > 1}">
            <div class="Pagination" id="Pagination">
                <c:if test="${requestScope.pageArticle.page > 1}">
                    <i class="iconfont icon-jiantou Pagination-left" name="${requestScope.pageArticle.page - 1}" onclick="paginationLeft(this)"></i>
                </c:if>
                <input type="text" class="Pagination-input" name="${requestScope.pageArticle.totalPage}" placeholder="${requestScope.pageArticle.page}/${requestScope.pageArticle.totalPage}" maxlength="10">
                <c:if test="${requestScope.pageArticle.page < requestScope.pageArticle.totalPage}">
                    <i class="iconfont icon-jiantou Pagination-right" name="${requestScope.pageArticle.page + 1}" onclick="paginationRight(this)"></i>
                </c:if>
            </div>
        </c:if>
    </div>
